...
import { Approval, ApprovalA, Claiming, initWeb3,MintA, Power, Staking } from "../state/ui";
...
const Home = () => {
  ...
  const [AvaReq, setAvaReq] = useState(0);
  ...
  const WhiteList = useSelector((state)=>{
    return state.adoptReducer.WhiteList;
  });
...
const balanceNFT = useSelector((state)=>{
  return state.adoptReducer.balanceNFT;
});
...
async function handleBulkMint(qty){
 ...
 dispatch(MintA({qty,AvaReq,tokenApproval,classes,traits,ImgUri,series}))
}
...
const handleApprove = ()=>{
  dispatch(ApprovalA({qty: tokenApproval}))
}
...
var MintPrice = WhiteList &&  WhiteList? 2 : 1
var threshold1 = 10
var threshold2 = 15
var threshold3 = 20
var threshold4 = 25
...
var tokenPriceT1 = 50
var tokenPriceT2 = 100
var tokenPriceT3 = 200

function calculatateTokenApproval(_qty){
 
var stakedBalance = balanceNFT &&  Number(balanceNFT) + _qty
var priceReq = 0
var tokenReq = 0

if(stakedBalance<=threshold1){
  console.log("A ")
  setAvaReq(_qty * MintPrice)
  settokenApproval(0)
   
}

else if (stakedBalance <= threshold2){
    if(balanceNFT <= threshold1){
      console.log("B ")
      priceReq = (threshold1 - balanceNFT) * MintPrice 
      setAvaReq(priceReq)
      tokenReq = (stakedBalance - threshold1) * tokenPriceT1
      settokenApproval(tokenReq)

    } else {


      console.log("C ")
      settokenApproval(_qty * tokenPriceT1)
    } 
}

else if (stakedBalance <= threshold3){
  if(balanceNFT <= threshold1){
    console.log("D ")
    priceReq = (threshold1 - balanceNFT) * MintPrice 
    setAvaReq(priceReq)
    tokenReq =   (threshold2 - threshold1) * tokenPriceT1  +(stakedBalance - threshold2) * tokenPriceT2
    settokenApproval(tokenReq)}
  
  else if(balanceNFT <= threshold2){

    console.log("E ")
    tokenReq = (threshold2 -  balanceNFT) * tokenPriceT1 + (stakedBalance - threshold2) * tokenPriceT2
    settokenApproval(tokenReq)
    ...
    console.log("F ")
    settokenApproval(_qty * tokenPriceT2)
  } 
}

else if (stakedBalance <= threshold4){
  if(balanceNFT <= threshold1){
    console.log("G ")
    priceReq = (threshold1 - balanceNFT) * MintPrice 
    setAvaReq(priceReq)
    tokenReq = (threshold1-balanceNFT) * 0 + (threshold2-threshold1) * tokenPriceT1 +  (threshold3-threshold2) * tokenPriceT2 + (stakedBalance-threshold3) * tokenPriceT3     
    ...
    tokenReq = (threshold2-balanceNFT) * tokenPriceT1 + (threshold3-threshold2) * tokenPriceT2 +  (stakedBalance-threshold3) * tokenPriceT3    


    settokenApproval(tokenReq)

  } 
  else if(balanceNFT <= threshold3){
    console.log("I ")

    tokenReq = (threshold3-balanceNFT) * tokenPriceT2 +   (stakedBalance-threshold3) * tokenPriceT3
    settokenApproval(tokenReq)
    ...
    console.log("J ")
    settokenApproval(_qty * tokenPriceT3)
  } 
} 
  ...
  if(balanceNFT <= threshold1){
    ...
    console.log("statement 1",(threshold2-balanceNFT) * tokenPriceT1)
    console.log("statement 2",(threshold3-threshold2) * tokenPriceT2 )
    console.log("statement 3",(threshold4-threshold3) * tokenPriceT3)
    console.log("statement 4",(stakedBalance-threshold3) * tokenPriceT3)
    settokenApproval(tokenReq)
...
console.log("ava req",AvaReq)
console.log("token approval set",tokenApproval)
            ...
            <div className="inc-sec flex aic">
              <div
                ...
                onClick={(e) => {
                    ...
                    setNumb(numb-1);
                    calculatateTokenApproval(numb-1)
                    if(numb-1<0){
...
{tokenApproval ==0  || Approval >=tokenApproval ?        <button 
            onClick={()=>{
            ...
            <button
            onClick={()=>{handleApprove(tokenApproval)} }
            >Approval</button>
            }
